...
def send_confirmation_email(email,token):
    host=os.environ.get('SMTP_HOST','');user=os.environ.get('SMTP_USER','');password=os.environ.get('SMTP_PASSWORD','');sender=os.environ.get('SMTP_FROM',user)
    if not host or not sender:return False
    link=urljoin(os.environ.get('PUBLIC_BASE_URL',request.url_root),url_for('member_confirm',token=token).lstrip('/'))
...
def member_register():
    if request.method=='POST':
        ...
        if not re.match(r'^[^@\s]+@[^@\s]+\.[^@\s]+$',email):flash('Informe um e-mail válido.','error')
            ...
            try:sent=send_confirmation_email(email,token)
            except Exception:sent=False
            if sent:flash('Cadastro criado! Enviamos um link de confirmação para seu e-mail.','success')
            else:flash('Conta criada, mas o envio de e-mail ainda não está configurado. Peça ao administrador para configurar o SMTP.','error')
...
def member_resend_confirmation():
    ...
    if account and not account['email_verified']:
        ...
        try:send_confirmation_email(email,token)
...
def member_profile(account_id):
    member=get_db().execute("""SELECT a.*,p.nickname,p.avatar_url,p.avatar_file,p.aomstats_url,p.elo_1v1,p.elo_team,p.quote FROM member_accounts a LEFT JOIN players p ON p.id=a.player_id WHERE a.id=? AND a.email_verified=1""",(account_id,)).fetchone()
    ...
    if member['player_id']:history=get_db().execute("""SELECT d.*,a.nickname challenger,b.nickname challenged,w.nickname winner FROM duels d JOIN players a ON a.id=d.challenger_id JOIN players b ON b.id=d.challenged_id LEFT JOIN players w ON w.id=d.winner_id WHERE d.challenger_id=? OR d.challenged_id=? ORDER BY d.id DESC""",(member['player_id'],member['player_id'])).fetchall()
    return render_template('member_profile.html',member=member,online=online,history=history)
...
def community_hub():
    members=get_db().execute("""SELECT a.id,a.player_id,a.description,a.last_seen,p.nickname,p.avatar_url,p.avatar_file,p.elo_1v1,p.quote,(a.last_seen>=datetime('now','-5 minutes')) online FROM member_accounts a LEFT JOIN players p ON p.id=a.player_id WHERE a.email_verified=1 ORDER BY online DESC,LOWER(COALESCE(p.nickname,a.email))""").fetchall();duels=get_db().execute("""SELECT d.*,a.nickname challenger,b.nickname challenged,w.nickname winner,ca.id challenger_account,cb.id challenged_account FROM duels d JOIN players a ON a.id=d.challenger_id JOIN players b ON b.id=d.challenged_id LEFT JOIN players w ON w.id=d.winner_id LEFT JOIN member_accounts ca ON ca.player_id=d.challenger_id LEFT JOIN member_accounts cb ON cb.player_id=d.challenged_id ORDER BY CASE d.status WHEN 'em_curso' THEN 0 WHEN 'solicitado' THEN 1 ELSE 2 END,d.id DESC LIMIT 50""").fetchall();return render_template('community_hub.html',members=members,duels=duels)
...
def map_download(map_id):
    ...
    db.execute('UPDATE community_maps SET downloads=downloads+1 WHERE id=?',(map_id,));db.commit()
    return send_from_directory(UPLOAD_DIR/'maps',row['file_name'],as_attachment=True)
...
@admin_required
def admin_map_add():
    require_csrf();archive=request.files.get('map_file');image=request.files.get('map_image')
    ext=Path(archive.filename or '').suffix.lower() if archive else ''
    if ext not in ('.zip','.rar','.7z'):flash('Envie um mapa ZIP, RAR ou 7Z.','error');return redirect(url_for('admin'))
    folder=UPLOAD_DIR/'maps';folder.mkdir(parents=True,exist_ok=True);file_name=f"{uuid.uuid4().hex}{ext}";archive.save(folder/file_name);image_name=''
    if image and Path(image.filename or '').suffix.lower() in ALLOWED_IMAGE_EXTENSIONS:image_name=f"{uuid.uuid4().hex}{Path(image.filename).suffix.lower()}";image.save(folder/image_name)
    get_db().execute('INSERT INTO community_maps(name,creator,category,file_name,image_name) VALUES(?,?,?,?,?)',(request.form.get('name','')[:120],request.form.get('creator','')[:120],request.form.get('category','FFA'),file_name,image_name));get_db().commit();flash('Mapa publicado.','success');return redirect(url_for('admin'))

@app.post('/admin/mapas/<int:map_id>/remover')
@admin_required
...
@app.get("/health")
def health():
    return {"version":"12-social-steam","database":"ok"}


init_db()
migrate_v6_db()
ensure_default_admin()
print("🔥 CHAMAS FLAMEJANTES V12 — SOCIAL + STEAM\nDATABASE: SQLITE\nSTATUS: READY",flush=True)
